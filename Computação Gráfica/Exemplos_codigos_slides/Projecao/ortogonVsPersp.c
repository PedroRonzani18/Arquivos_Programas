...
void configuraProjecao() // determina se é p ou o e muda para ortho ou frustrum
{
  ...
  {
  case 'p':
      glFrustum(-razaoAspecto, razaoAspecto, -1.0, 1.0, 2.0, 100.0); // projedção perspectiva
      break;
  case 'o':
      glOrtho(-3*razaoAspecto, 3*razaoAspecto, -3, 3, -100, 100); // projeção ortogonal
      break;
  }
...
void desenha() // translada, rotaciona, desenha
{
    configuraProjecao();
    ...
    glPushMatrix();
        ...
        glRotated(60,1,0,0);
        glRotated(a,0,0,1);
        glutSolidSphere(1,slices,stacks); // raio, longitude, latitude
    glPopMatrix();

    // CONE sólido
    glPushMatrix();
        glTranslated(0,1.2,-6);
        glRotated(60,1,0,0); //rotaciona para deixar mais visivel
        glRotated(a,0,0,1); // toraciona toda hora
...
int main(int argc, char *argv[])
{
    ...
    glEnable(GL_CULL_FACE); // não renderiza imagens que não estão sendo vistas (costas de um cubo)
    glCullFace(GL_BACK);
